...
clear all;
close all;
% load('C:\Users\ax\Documents\博士作业\HW2\indy_20161005_06.mat');
load('C:\Users\ax\Documents\博士作业\HW2\indy_20160407_02.mat');
...
X_train = spike_rate(1:num_train, 2:end)';
Y_train = pos_x_ds(1:num_train)';
X_test = spike_rate(num_train+1:num_train+num_test, 2:end)';
...
options = trainingOptions('adam', 'MaxEpochs', 50, 'GradientThreshold', 1, 'MiniBatchSize', 64);
...
% predict(net, X_test, 'MiniBatchSize', 1)
test_p_x=predict(net, X_test, 'MiniBatchSize', 1);
figure()
subplot(2,3,1)
...
layers = [ ...
    ...
    fullyConnectedLayer(numResponses)
    regressionLayer];
net = trainNetwork(X_train, v_x_ds(1:num_train)', layers, options);
% test_v_x=[ones(num_test,1),spike_rate(num_train+1:num_train+num_test,2:end)]*b_v_x;
test_v_x=predict(net, X_test, 'MiniBatchSize', 1);
subplot(2,3,4)
...
layers = [ ...
    ...
    fullyConnectedLayer(numResponses)
    regressionLayer];
net = trainNetwork(X_train, a_x_ds(1:num_train)', layers, options);
% test_v_x=[ones(num_test,1),spike_rate(num_train+1:num_train+num_test,2:end)]*b_v_x;
test_a_x=predict(net, X_test, 'MiniBatchSize', 1);
...
layers = [ ...
    ...
    fullyConnectedLayer(numResponses)
    regressionLayer];
net = trainNetwork(X_train, pos_y_ds(1:num_train)', layers, options);
% test_v_x=[ones(num_test,1),spike_rate(num_train+1:num_train+num_test,2:end)]*b_v_x;
test_p_y=predict(net, X_test, 'MiniBatchSize', 1);
...
layers = [ ...
    ...
    fullyConnectedLayer(numResponses)
    regressionLayer];
net = trainNetwork(X_train, v_y_ds(1:num_train)', layers, options);
% test_v_x=[ones(num_test,1),spike_rate(num_train+1:num_train+num_test,2:end)]*b_v_x;
test_v_y=predict(net, X_test, 'MiniBatchSize', 1);
...
layers = [ ...
    ...
    fullyConnectedLayer(numResponses)
    regressionLayer];
net = trainNetwork(X_train, a_y_ds(1:num_train)', layers, options);
% test_v_x=[ones(num_test,1),spike_rate(num_train+1:num_train+num_test,2:end)]*b_v_x;
test_a_y=predict(net, X_test, 'MiniBatchSize', 1);
